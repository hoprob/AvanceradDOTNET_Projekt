 ...
 *Jag har nu läst på om och implementerat AutoMapper i mitt projekt. AuttoMapper är verkligen användbart för att kunna forma och styra vilken data som skickas med i Json filen i en response. Det är bra dels för att man slipper onödig dagta och får en resn och snygg Json till Frontend, men också för att inte behöva visa data som kan vara känslig eller onödig över http protokollet. Med andra ord så kan man skapa en säkrare och tydligare applikation med hjälp av AutoMapper. Det är en klar fördel att använda AutoMapper för strukturen och klarheten i koden. Genom att skapa modeller och kopiera till desssa genom AutoMapepr så slipper man stegen att antingen kopiera property för property själv mellan objekt eller att returnera alternativa objekt inom programmet. AutoMapper sköter mappningen i bara en rad och man kan använda sina ordinarie objekt hela vägen tills det att de skall skickas iväg. Den enda nackdelen som jag ser med AutoMapper är att det blir fler klasser och mappar i projektet, vilket gör att det kan bli mer komplicerat att förstå och sätta sig in i projektet. Men i det stora hela så tycker jag att fördelarna i detta fall övervinner nackdelarna.*
